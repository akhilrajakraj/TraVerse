...
export function TripAIPlannerPanel({ tripId }: TripAIPlannerPanelProps) {
  ...
  return (
    <section className="mt-8 border-t border-[var(--line)] pt-6" aria-labelledby="ai-planner-heading">
      ...
      {showInitialStatusError ? <ErrorState title="Planner status unavailable" message={statusError instanceof Error ? statusError.message : "We couldn't retrieve the AI planner status."} onRetry={() => void statusQuery.refetch()} /> : null}
        ...
        <Card>
          ...
          {status.status === "needs_review" ? (
            <div className="mt-4 rounded-lg border border-warning bg-warning-bg p-4">
              ...
              {status.error_message ? <details className="mt-3 text-sm text-neutral"><summary className="cursor-pointer font-medium">Show technical diagnostic</summary><pre className="mt-2 overflow-x-auto whitespace-pre-wrap rounded-md bg-[var(--surface-solid)] p-3 text-xs">{status.error_message}</pre></details> : null}
            </div>
          ) : null}
          {status.status === "failed" && status.error_message ? <p className="mt-4 text-sm text-danger" role="alert">{status.error_message}</p> : null}
          {status.status === "succeeded" ? <p className="mt-4 text-sm text-neutral">Itinerary, budget, and recommendation panels will refresh from the authoritative server state.</p> : null}
        </Card>
      ) : null}
      {showQueuedStatus ? <Card className="mt-4"><p className="font-semibold">Planning request queued</p><p className="mt-1 text-sm text-neutral">The planner has accepted the request. Waiting for the asynchronous Agent Run to appear.</p></Card> : null}
      {pollUntil !== null && !isActive && !isTerminal && !statusQuery.isLoading ? <p className="mt-3 text-sm text-neutral" role="status">Waiting for the planner worker to start...</p> : null}
      {trigger.isError ? <div className="mt-4" role="alert"><ErrorState title="Unable to start AI planning" message={trigger.error instanceof Error ? trigger.error.message : "We couldn't queue the planning request."} /></div> : null}
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Button type="button" onClick={handleTrigger} isLoading={trigger.isPending} disabled={trigger.isPending || Boolean(isActive) || Boolean(pollUntil)}>{isTerminal ? "Retry AI planner" : "Generate AI trip plan"}</Button>
