...
let triggerState: { isPending: boolean; isSuccess: boolean; isError: boolean; error: Error | null };
...
describe("TripAIPlannerPanel", () => {
  ...
  it("renders an active backend-controlled AgentRun status", () => {
    statusState = {
      ...statusState,
      data: { id: "run-1", agent_type: "travel_planner", status: "running", error_message: "", started_at: "2026-09-01T10:00:00Z", completed_at: null },
      isError: false,
      error: null,
    };
    render(<TripAIPlannerPanel tripId="trip-1" />);
  ...
  it("makes needs_review actionable and keeps the parser diagnostic secondary", () => {
    statusState = {
      ...statusState,
      data: { id: "run-1", agent_type: "travel_planner", status: "needs_review", error_message: "Unable to produce valid structured output. Initial error: malformed JSON", started_at: "2026-09-01T10:00:00Z", completed_at: "2026-09-01T10:02:00Z" },
      isError: false,
      error: null,
    };
    render(<TripAIPlannerPanel tripId="trip-1" />);
  ...
  it("refreshes authoritative trip data after a successful planning run", () => {
    statusState = {
      ...statusState,
      data: { id: "run-1", agent_type: "travel_planner", status: "succeeded", error_message: "", started_at: "2026-09-01T10:00:00Z", completed_at: "2026-09-01T10:02:00Z" },
      isError: false,
      error: null,
    };
    render(<TripAIPlannerPanel tripId="trip-1" />);
