  ...
  it("shares one refresh request across concurrent 401 responses", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/token/refresh/")) {
        ...
        return new Response(JSON.stringify({ access: "fresh", refresh: "refresh-2" }), { status: 200 });
      }
      const auth = (input as Request).headers?.get?.("Authorization");
      if (auth === "Bearer fresh") return new Response(JSON.stringify({ ok: true }), { status: 200 });
      return new Response(JSON.stringify({ detail: "expired" }), { status: 401 });
    }));

    const { apiRequest } = await import("../apiClient");
    const [first, second] = await Promise.all([apiRequest<{ ok: boolean }>("/api/a"), apiRequest<{ ok: boolean }>("/api/b")]);
