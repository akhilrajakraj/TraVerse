...
def create_share_link(
    *,
    ...
    """
    Create a shareable document link for a trip.
    """
...
def revoke_share_link(
    *,
    ...
    """
    Revoke an active shareable document link.
    """
...
def generate_itinerary_pdf(
    *,
    ...
    The generated PDF is not persisted to disk or database.
    """
    days = get_trip_itinerary(trip=trip)
    serialized_days = ItineraryDaySerializer(
        days,
        many=True,
    ).data
        ...
        for item in day.get("items", []):
            ...
            if item.get("estimated_cost_usd") is not None:
                pdf.drawString(
                    70,
                    y,
                    f"Estimated cost: ${item['estimated_cost_usd']}",
                )
                y -= 14
